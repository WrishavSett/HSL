...
# Initialised once at startup; raises immediately if env vars are missing or config is invalid.
...
async def extract(File: UploadFile = FastAPIFile(...)) -> JSONResponse:
    ...
    **Errors**
    ...
    if not pdf_bytes:
        raise HTTPException(status_code=400, detail="Uploaded file is empty. Send a valid PDF file.")
