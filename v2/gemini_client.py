...
from error import (
    GoogleGenAINotInstalledError,
    GeminiMissingAPIKeyError,
    GeminiMissingModelNameError,
    GeminiAuthError,
    GeminiModelNotFoundError,
    GeminiImageNotFoundError,
...
try:
    ...
    raise GoogleGenAINotInstalledError()
...
class GeminiClient:
    def __init__(
        ...
        if not api_key:
            ...
            raise GeminiMissingAPIKeyError()

        if not model_name:
            log.critical("GEMINI_MODEL_NAME environment variable is not set.")
            raise GeminiMissingModelNameError()
    ...
    def _auth(self) -> None:
        try:
            ...
            if self.model_name not in model_names:
                ...
                raise GeminiModelNotFoundError(self.model_name, model_names)
            log.info(f"Gemini client initialised with model '{self.model_name}'.")
        except GeminiModelNotFoundError:
            raise
    ...
    def call_llm(
        ...
        for image_path in image_paths:
            if not os.path.exists(image_path):
                ...
                raise GeminiImageNotFoundError(image_path)
            try:
                with open(image_path, "rb") as f:
                    image_bytes = f.read()
            except Exception as exc:
                log.error(f"Could not read image file {image_path!r}: {exc}")
                raise GeminiImageReadError(image_path, str(exc)) from exc
        ...
        try:
            ...
            raise GeminiAPICallError(str(exc)) from exc
