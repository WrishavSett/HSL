...
from errors import HSLGeminiError
...
class GeminiClient:
    ...
    ------
    HSLGeminiError
        If ``api_key`` or ``model_name`` is empty, if ``model_name`` is not
        in the list of available Gemini models, or if the API key validation
        request fails.
    ...
    def __init__(
        ...
        model_name = model_name or os.getenv("GEMINI_MODEL_NAME")

        if not api_key:
            log_msg = "GEMINI_API_KEY environment variable is not set."
        ...
        if not model_name:
            log_msg = "GEMINI_MODEL_NAME environment variable is not set."
            log.critical(log_msg)
            raise HSLGeminiError(
                message="Internal server error.",
                error=log_msg,
    ...
    def _auth(self) -> None:
        ...
        HSLGeminiError
            ...
            models, or if the ``client.models.list()`` call fails.
        """
        try:
        ...
        except HSLGeminiError:
            raise
        except Exception as exc:
            log_msg = f"Gemini API key validation failed: {exc}"
            log.critical(log_msg)
            raise HSLGeminiError(
                message="Internal server error.",
                error=log_msg,
    ...
    def call_llm(
        ...
        ------
        HSLGeminiError
            If any image file is missing or unreadable, or if the Gemini API
            call fails.
        ...
        for image_path in image_paths:
            ...
            try:
                ...
                log_msg = f"Could not read image file {image_path!r}: {exc}"
                log.error(log_msg)
                raise HSLGeminiError(
                    message="Internal server error.",
                    error=log_msg,
        ...
        try:
            ...
            log_msg = f"Gemini API call failed: {exc}"
            log.error(log_msg)
            raise HSLGeminiError(
                message="Internal server error.",
                error=log_msg,
    ...
    def extract_invoice_data(
        ...
        ------
        HSLConfigError
            If the config file is invalid (propagated from
            :func:`helper.load_config`).
        HSLPDFError
            If PDF conversion fails (propagated from
            :func:`helper.pdf_to_images`).
        HSLGeminiError
            If the Gemini API call fails or ``response.parsed`` is ``None``.
