...
from errors import HSLError, HSLStorageError
...
def _error_response(message: str, error: str, status_code: int) -> JSONResponse:
    ...
    ----------
    message : str
        User-facing summary (the raise message).
    error : str
        Full operator-facing detail (the log message).
    status_code : int
        HTTP status code.
...
def _success_response(data: dict) -> JSONResponse:
    ...
    ----------
    data : dict
        Resolved extraction fields.

    Returns
    -------
    JSONResponse
        ``{"success": true, "message": null, "error": null, "data": {...}}``
    """
    return JSONResponse(
        content={
            "success": True,
...
# ---------------------------------------------------------------------------
# Global exception handler — FastAPI-generated HTTPExceptions
# ---------------------------------------------------------------------------

@app.exception_handler(FastAPIHTTPException)
async def http_exception_handler(request, exc: FastAPIHTTPException) -> JSONResponse:
    """
    Map FastAPI-generated ``HTTPException`` instances to the standard envelope.

    Covers framework-level errors such as missing form fields, wrong HTTP
    method, and unrecognised routes — all of which bypass the ``extract``
    handler and would otherwise return unstructured responses.
    """
...
def _save_upload(pdf_bytes: bytes) -> str:
    ...
    try:
        ...
        log_msg = f"Failed to write temporary PDF file at {temp_path}: {exc}"
        log.error(log_msg)
...
@app.post(
    ...
    response_description="Structured JSON: success, message, error, data",
)
async def extract(File: UploadFile = FastAPIFile(...)) -> JSONResponse:
    ...
    pipeline, and returns a structured JSON envelope.  The temporary file is
    always deleted after extraction, regardless of outcome.
    ...
    500 Internal Server Error
        Any server-side or pipeline failure.
    ...
    if not is_pdf_content_type and not is_pdf_extension:
        ...
        log.error(log_msg)
        return _error_response(message=log_msg, error=log_msg, status_code=415)

    # 2. Read uploaded file
    try:
        pdf_bytes = await File.read(_MAX_UPLOAD_SIZE + 1)
    except Exception as exc:
        log_msg = f"Failed to read uploaded file: {exc}"
        log.error(log_msg)
        return _error_response(message="Internal server error.", error=log_msg, status_code=500)

    if not pdf_bytes:
        log_msg = "Uploaded file is empty. Send a valid PDF file."
        log.error(log_msg)
        return _error_response(message=log_msg, error=log_msg, status_code=400)

    if len(pdf_bytes) > _MAX_UPLOAD_SIZE:
        log_msg = f"Uploaded file exceeds maximum size of {_MAX_UPLOAD_SIZE} bytes."
        log.error(log_msg)
        return _error_response(message=log_msg, error=log_msg, status_code=413)
    ...
    try:
        ...
        return _error_response(message=exc.message, error=exc.error, status_code=500)

    except Exception as exc:
        log_msg = f"Extraction failed: {exc}"
        log.error(log_msg)
        return _error_response(message="Internal server error.", error=log_msg, status_code=500)
    ...
    # 4. Resolve fields and return structured response
    try:
        data = resolve_paths(result, _fields_of_interest)
    except Exception as exc:
        log_msg = f"Field resolution failed: {exc}"
        log.error(log_msg)
        return _error_response(message="Internal server error.", error=log_msg, status_code=500)
