...
def normalize_subtotal(value: str) -> str:
    ...
    Gemini may return subtotals in a variety of formats depending on the
    source document — with or without currency symbols, grouping commas,
    suffixes such as ``/-``, or surrounding whitespace. Rather than
    enumerating every possible prefix or suffix, this function locates the
    numeric pattern directly and discards everything else.

    The pattern ``r'[\\d,]+\\.\\d+'`` matches one or more digits or commas
    followed by a decimal point and at least one digit. Indian-style grouping
    commas (e.g. ``4,91,716.95``) are removed from the match before returning.

    If no numeric pattern is found (e.g. Gemini returns ``"N/A"`` or an empty
    ...
    Returns:
        str: Plain numeric string (e.g. ``"491716.95"``), or the original
             *value* if no numeric pattern could be found.
